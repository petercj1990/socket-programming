...
if ARGV[1] == "random"
  s.getline  #get hello message from server
  for x in 1..25 do
    s.random()  #send 25 random requests
  end
  s.send_expect_socket_close("adios")
elsif ARGV[1] == "evil"
  #send two requests right after each other without waiting for a response
  s.getline  #get hello message from server
  s.send_with_rn("help\r\nhelp")
  s.getline  #get response to first help request
  s.getline  #get response to second help request

  #send a partial request followed by a pause follwed by the rest of the request
  s.send_raw("hel")
  sleep(5)
  s.send_with_rn("p")
  s.getline  #get response to help request

  #send thre bogus \ns followed by a bogus\r\n
  s.send_raw("bogus\n")
  s.send_raw("bogus\n")
  s.send_raw("bogus\n")
  s.send_receive("bogus")   #get response to three bogus \ns followed by a bogus \r\n
  s.send_expect_socket_close("adios")
else #normal
  s.getline
